import tensorflow as tf 
import sys,time,os
import numpy as np
from sklearn.metrics import precision_score, recall_score, f1_score
from util import load_w2v, batch_index, load_word_embedding, load_inputs
FLAGS=tf.app.flags.FLAGS
tf.app.flags.DEFINE_integer('embedding_dim',300,'dimension of word embedding')
tf.app.flags.DEFINE_integer('magic_m ',3,'wtf m')
tf.app.flags.DEFINE_integer('batch_size', 25, 'number of example per batch')
tf.app.flags.DEFINE_integer('n_hidden', 100, 'number of hidden unit')
tf.app.flags.DEFINE_float('learning_rate', 0.01, 'learning rate')
tf.app.flags.DEFINE_integer('n_class', 2, 'number of distinct class')
tf.app.flags.DEFINE_integer('max_sentence_len', 80, 'max number of tokens per sentence')
tf.app.flags.DEFINE_float('l2_reg', 0.007, 'l2 regularization')
tf.app.flags.DEFINE_integer('display_step', 4, 'number of test display step')
tf.app.flags.DEFINE_integer('n_iter', 25, 'number of train iter')
tf.app.flags.DEFINE_float('init', 0.2, 'init')
tf.app.flags.DEFINE_string('train_file_path', 'data/restaurant/data_train', 'training file')
tf.app.flags.DEFINE_string('validate_file_path', 'data/restaurant/rest_2014_test.txt', 'validating file')
tf.app.flags.DEFINE_string('test_file_path', 'data/restaurant/data_test', 'testing file')
tf.app.flags.DEFINE_string('embedding_file_path', 'data/restaurant/rest_2014_lstm_word_embedding_42b_300.txt', 'embedding file')
tf.app.flags.DEFINE_string('word_id_file_path', 'data/restaurant/word_id_new.txt', 'word-id mapping file')
tf.app.flags.DEFINE_string('aspect_id_file_path', 'data/restaurant/aspect_id_new.txt', 'word-id mapping file')
tf.app.flags.DEFINE_string('method', 'AT', 'model type: AE, AT or AEAT')
tf.app.flags.DEFINE_string('t', 'last', 'model type: ')
count = 0
class LSTM(object):
    """docstring for LSTM"""
    def __init__(self,magic_m=3, embedding_dim=300, batch_size=64, n_hidden=300, learning_rate=0.01,
                 n_class=3, max_sentence_len=50, l2_reg=0., display_step=4, n_iter=100, type_=''):
        self.magic_m=magic_m
        self.embedding_dim = embedding_dim
        self.batch_size = batch_size
        self.n_hidden = n_hidden
        self.learning_rate = learning_rate
        self.n_class = n_class
        self.max_sentence_len = max_sentence_len
        self.l2_reg = l2_reg
        self.display_step = display_step
        self.n_iter = n_iter
        self.type_ = type_
        self.word_id_mapping,self.w2v=load_word_embedding(FLAGS.word_id_file_path, FLAGS.embedding_file_path, self.embedding_dim)
        self.word_embedding = tf.Variable(self.w2v, dtype=tf.float32, name='word_embedding')

        self.keep_prob1 = tf.placeholder(tf.float32)
        self.keep_prob2 = tf.placeholder(tf.float32)   
        
        with tf.name_scope('inputs'):
            self.x = tf.placeholder(tf.int32, [None, self.max_sentence_len], name='x')
            self.y = tf.placeholder(tf.int32, [None, self.max_sentence_len,self.n_class], name='y')
            self.sen_len = tf.placeholder(tf.int32, None, name='sen_len')
            self.aspect_id = tf.placeholder(tf.int32, None, name='aspect_id')

        with tf.name_scope('weights'):
            self.weights = {
                'softmax': tf.get_variable(
                    name='softmax_W',
                    shape=[self.n_hidden, self.n_class],
                    initializer=tf.random_uniform_initializer(-FLAGS.init, FLAGS.init),
                    regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg)
                )
            }

        with tf.name_scope('biases'):
            self.biases = {
                'softmax': tf.get_variable(
                    name='softmax_biases',
                    shape=[self.n_class],
                    initializer=tf.random_uniform_initializer(-FLAGS.init, FLAGS.init),
                    regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg)
                ),
                'b1':tf.get_variable(
                    name='b1',
                    shape=[1],
                    initializer=tf.random_uniform_initializer(-FLAGS.init, FLAGS.init),
                    regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg)
                ),
                'b2':tf.get_variable(
                    name='b2',
                    shape=[self.n_hidden + self.embedding_dim],
                    initializer=tf.random_uniform_initializer(-FLAGS.init, FLAGS.init),
                    regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg)
                )
            }



    def dynamic_rnn(self,cell,inputs,length,max_len,scope_name,out_type='all'):
        outputs,state=tf.nn.dynamic_rnn(
            cell(self.n_hidden),
            inputs=inputs,
            sequence_length=length,
            dtype=tf.float32,
        )  # outputs -> batch_size * max_len * n_hidden
        # batch_size = tf.shape(outputs)[0]

        return outputs



    def model(self,inputs,type_=''):
        # inputs shape 25 80 900
        batch_size=tf.shape(inputs)[0]
        cell = tf.contrib.rnn.LSTMCell
        hiddens = self.dynamic_rnn(cell, inputs, self.sen_len, self.max_sentence_len, 'all')

        self.hidden_shape=tf.shape(hiddens)
        # self.traget_shape=tf.shape(target)
        # hiddens=tf.matmul(tf.reshape(hiddens,[-1,2*self.n_hidden]),self.trans_w)
        # self.new_hidden_shape=tf.shape(hiddens)
        # hiddens=tf.reshape(hiddens,[self.batch_size,self.max_sentence_len,self.n_hidden])
        h_m=tf.matmul(tf.reshape(outputs,[-1,n_hidden]),self.weights['softmax'])+self.biases['softmax']
        h=tf.reshape(h_m,[-1,max_len,2])
        return tf.nn.softmax(h)
        # h_t = tf.reshape(tf.concat([hiddens, target],2), [-1, self.n_hidden + self.embedding_dim])

        # # M = tf.matmul(tf.nn.relu(tf.matmul(h_t, self.W)+self.biases['b2']), self.w)+self.biases['b1']
        # M = tf.nn.tanh(tf.matmul(h_t, self.W)+self.biases['b1'])    
        # self.m_shape=tf.shape(M)

        # alpha = LSTM.softmax(tf.reshape(M, [-1, 1, self.max_sentence_len]), self.sen_len, self.max_sentence_len)
        # self.alpha = tf.reshape(alpha, [-1, self.max_sentence_len])

        # r = tf.reshape(tf.matmul(alpha, hiddens), [-1, self.n_hidden])
        # index = tf.range(0, batch_size) * self.max_sentence_len + (self.sen_len - 1)
        # hn = tf.gather(tf.reshape(hiddens, [-1, self.n_hidden]), index)  # batch_size * n_hidden

        # h = tf.nn.relu(tf.matmul(hiddens, self.Wx))

        # return LSTM.softmax_layer(h, self.weights['softmax'], self.biases['softmax'], self.keep_prob2)
      

    @staticmethod
    def reduce_mean(inputs, length):
        """
        :param inputs: 3-D tensor
        :param length: the length of dim [1]
        :return: 2-D tensor
        """
        length = tf.cast(tf.reshape(length, [-1, 1]), tf.float32) + 1e-9
        inputs = tf.reduce_sum(inputs, 1, keep_dims=False) / length
        return inputs
 
    def run(self):
         # print(shape(self.x)
        inputs = tf.nn.embedding_lookup(self.word_embedding, self.x)
        print('inputs shape is :' ,np.shape(inputs))
        tmp_zero=np.array([0.]*self.embedding_dim*self.batch_size*self.max_sentence_len,dtype=np.float32)
        # tmp=np.array([0.]*self.embedding_dim,dtype=np.float32)
        # print('tmp_zero shape is :' ,np.shape(tmp_zero))
        # for i in range(int(self.magic_m/2)-1):
        #     tmp=np.vstack((tmp,tmp_zero))
        #     print('tmp shape is :' ,np.shape(tmp))

        # print('np.vstack((tmp,inputs)) shape is :' ,np.shape(np.vstack((tmp,inputs))))
        tmp=tf.Variable(np.reshape(tmp_zero,[-1,self.max_sentence_len,self.embedding_dim]))
        # self.tmp=tf.reshape(self.tmp,[1,self.embedding_dim])
        # for k in range(self.batch_size):
            # tmp_inputs=tf.concat([tf.concat([self.tmp,inputs[k]],0),self.tmp],0)
            # # final_inputs=tf.Variable(None)
            # for i in range(self.max_sentence_len):
            #     for j in range(1,self.magic_m):
            #         inputs[k,i]=tf.concat([inputs[k,i],tmp_inputs[i+j] ],0 )

        tmp1 =tf.concat([tf.concat([tmp,inputs],1), tmp],1 ) #25 82 300

        final_inputs_f=tf.concat([tmp1[:,:self.max_sentence_len,:], tmp1[:,1:self.max_sentence_len+1,:]],2)
        final_inputs_fb=tf.concat([final_inputs_f, tmp1[:,2:self.max_sentence_len+2,:]],2)
        self.final_inputs_fb_shape=tf.shape(final_inputs_fb)
        # prob = self.model(final_inputs_fb)

        # with tf.name_scope('loss'):
        #     reg_loss = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
        #     cost = - tf.reduce_mean(self.y * tf.log(prob)) + sum(reg_loss)

        # with tf.name_scope('train'):
        #     global_step = tf.Variable(0, name="tr_global_step", trainable=False)
        #     optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(cost, global_step=global_step)

        # with tf.name_scope('predict'):
        #     correct_pred = tf.equal(tf.argmax(prob, 2), tf.argmax(self.y, 2))
        #     true_y = tf.argmax(self.y, 2)
        #     pred_y = tf.argmax(prob, 2)
        #     accuracy = tf.reduce_sum(tf.cast(correct_pred, tf.int32))
        #     _acc = tf.reduce_mean(tf.cast(correct_pred, tf.float32))

        with tf.Session() as sess:
        #     title = '-d1-{}d2-{}b-{}r-{}l2-{}sen-{}dim-{}h-{}c-{}'.format(
        #         FLAGS.keep_prob1,
        #         FLAGS.keep_prob2,
        #         FLAGS.batch_size,
        #         FLAGS.learning_rate,
        #         FLAGS.l2_reg,
        #         FLAGS.max_sentence_len,
        #         FLAGS.embedding_dim,
        #         FLAGS.n_hidden,
        #         FLAGS.n_class
        #     )
        #     summary_loss = tf.summary.scalar('loss' + title, cost)
        #     summary_acc = tf.summary.scalar('acc' + title, _acc)
        #     train_summary_op = tf.summary.merge([summary_loss, summary_acc])
        #     validate_summary_op = tf.summary.merge([summary_loss, summary_acc])
        #     test_summary_op = tf.summary.merge([summary_loss, summary_acc])
        #     import time
        #     timestamp = str(int(time.time()))
        #     _dir = 'logs/' + str(timestamp) + '_' + title
        #     train_summary_writer = tf.summary.FileWriter(_dir + '/train', sess.graph)
        #     test_summary_writer = tf.summary.FileWriter(_dir + '/test', sess.graph)
        #     validate_summary_writer = tf.summary.FileWriter(_dir + '/validate', sess.graph)

        #     saver = tf.train.Saver(write_version=tf.train.SaverDef.V2)

            init = tf.global_variables_initializer()
            sess.run(init)            

            # save_dir = 'models/' + _dir + '/'
            # import os
            # if not os.path.exists(save_dir):
            #     os.makedirs(save_dir)

            tr_x, tr_sen_len,  tr_y = load_inputs(
                FLAGS.train_file_path,
                self.word_id_mapping,
                self.max_sentence_len
            )
            te_x, te_sen_len,te_y = load_inputs(
                FLAGS.test_file_path,
                self.word_id_mapping,
                self.max_sentence_len
            )
            max_acc = 0.
            max_alpha = None
            max_ty, max_py = None, None
            for i in range(self.n_iter):
                acc, loss, cnt = 0., 0., 0
                for train, _ in self.get_batch_data(tr_x, tr_sen_len, tr_y, self.batch_size):
                    # print('hidden_shape',sess.run([self.in_t_shape, self.new_hidden_shape, self.m_shape, self.hidden_shape],feed_dict=train))
                    print(sess.run(self.final_inputs_fb_shape,feed_dict=train))
                    train_acc,_op, step, summary = sess.run([accuracy,optimizer, global_step, train_summary_op], feed_dict=train)
                    train_summary_writer.add_summary(summary, step)
                    acc+=train_acc
                    cnt+=_

                print ('Iter {}: train  acc={:.6f}'.format(i, acc / cnt))
                acc, loss, cnt = 0., 0., 0
                flag = True
                summary, step = None, None
                alpha = None
                ty, py = None, None
                for test, num in self.get_batch_data(te_x, te_sen_len, te_y, self.batch_size):
                    _loss, _acc, _summary, _step, ty, py = sess.run([cost, accuracy, validate_summary_op, global_step, true_y, pred_y],
                                                            feed_dict=test)
                    acc += _acc
                    loss += _loss * num
                    cnt += num
                    if flag:
                        summary = _summary
                        step = _step
                        flag = False
                        alpha = alpha
                        ty = ty
                        py = py
                print ('all samples={}, correct prediction={}'.format(cnt, acc))
                test_summary_writer.add_summary(summary, step)
                saver.save(sess, save_dir, global_step=step)
                print ('Iter {}: mini-batch loss={:.6f}, test acc={:.6f}'.format(i, loss / cnt, acc / cnt))
                if acc / cnt > max_acc:
                    max_acc = acc / cnt
                    max_ty = ty
                    max_py = py
            print ('P:', precision_score(max_ty, max_py, average=None))
            print ('R:', recall_score(max_ty, max_py, average=None))
            print ('F:', f1_score(max_ty, max_py, average=None))

            print ('Optimization Finished! Max acc={}'.format(max_acc))
            # print('tf.contrib.rnn.BasicLSTMCell')
            fp = open('weight.txt', 'w')
            for y1, y2, ws in zip(max_ty, max_py, max_alpha):
                fp.write(str(y1) + ' ' + str(y2) + ' ' + ' '.join([str(w) for w in ws]) + '\n')

            print ('keep_prob1={},keep_prob2={},Learning_rate={}, iter_num={}, batch_size={}, hidden_num={}, l2={}'.format(
                FLAGS.keep_prob1,
                FLAGS.keep_prob2,
                self.learning_rate,
                self.n_iter,
                self.batch_size,
                self.n_hidden,
                self.l2_reg
            ))


    def get_batch_data(self, x, sen_len, y, batch_size, ):
        for index in batch_index(len(y), batch_size, 1):
            feed_dict = {
                self.x: x[index],
                self.y: y[index],
                self.sen_len: sen_len[index],
            }
            yield feed_dict, len(index)

def main(_):
    # os.mkdir('result')
    # sys.stdout = open('result/'+'={}='.format(time.strftime("%Y-%m-%d-%X", time.localtime())).replace(':','-'),'w')
    
    print(time.strftime("%Y-%m-%d-%X", time.localtime()))
    lstm = LSTM(
        magic_m=3,
        embedding_dim=FLAGS.embedding_dim,
        batch_size=FLAGS.batch_size,
        n_hidden=FLAGS.n_hidden,
        learning_rate=FLAGS.learning_rate,
        n_class=FLAGS.n_class,
        max_sentence_len=FLAGS.max_sentence_len,
        l2_reg=FLAGS.l2_reg,
        display_step=FLAGS.display_step,
        n_iter=FLAGS.n_iter,
        type_=FLAGS.method
    )    
    lstm.run()


if __name__ == '__main__':
    tf.app.run()