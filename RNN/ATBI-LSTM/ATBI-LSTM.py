...
class LSTM(object):
    ...
    def __init__(self, embedding_dim=100, batch_size=64, n_hidden=100, learning_rate=0.01,
        ...
        self.trans_w = tf.get_variable(
            name='trans_w',
            shape=[2*self.n_hidden, self.n_hidden],
            initializer=tf.random_uniform_initializer(-FLAGS.init, FLAGS.init),
            regularizer=tf.contrib.layers.l2_regularizer(self.l2_reg)
        )
    ...
    def AT(self,inputs,target,type_=''):
        ...
        h = tf.nn.relu(tf.matmul(r, self.Wp) + tf.matmul(hn, self.Wx))
    ...
    def run(self):
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.n_iter):
                ...
                alpha = None
                ty, py = None, None
                for test, num in self.get_batch_data(te_x, te_sen_len, te_y, te_target_word, 25, 1.0, 1.0, False):
                    _loss, _acc, _summary, _step, ty, py,alpha = sess.run([cost, accuracy, validate_summary_op, global_step, true_y, pred_y,self.alpha],
                                                            feed_dict=test)
                    print('answer is , ',sess.run([true_y],feed_dict=test))
                    print('predict is , ',sess.run([pred_y],feed_dict=test))
