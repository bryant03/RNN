...
class LSTM(object):
    ...
    def AT(self,inputs,target,type_=''):
        ...
        cell = tf.contrib.rnn.LSTMCell
        self.test_in_t_shape=tf.shape(in_t)#25 80 600
        hiddens = self.dynamic_rnn(cell, in_t, self.sen_len, self.max_sentence_len, 'AT', 'all')

        self.hidden_shape=tf.shape(hiddens)#25 80 300
        self.traget_shape=tf.shape(target) # 25 300
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.n_iter):
                ...
                for train, _ in self.get_batch_data(tr_x, tr_sen_len, tr_y, tr_target_word, self.batch_size, FLAGS.keep_prob1, FLAGS.keep_prob2):
                    print('hidden_shape',sess.run([self.m_shape, self.hidden_shape, self.test_in_t_shape, self.test_target_shape,self.traget_shape],feed_dict=train))
